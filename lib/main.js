...
module.exports = async appx => {
    const {config, logger, client, api, app} = appx;
    ...
    api.get('/maps/metrics', async ctx => {
    ...
    api.get('/maps/api/*', async ctx => {
        ...
        .filter(key => ((!!key) && (key.toString().toLowerCase() !== 'key') && (key.toString().toLowerCase() !== 'sessiontoken')))
        .reduce((query, key) => {
